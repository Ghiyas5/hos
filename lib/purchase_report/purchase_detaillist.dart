...
class Purchase_Detail_Lisr extends StatefulWidget {
  final String dateFrom,dateTo,itemCode,custCode;

  Purchase_Detail_Lisr({@required this.dateFrom,@required this.dateTo, this.itemCode, this.custCode});

  @override
  _Purchase_Detail_LisrState createState() => _Purchase_Detail_LisrState();
}

class _Purchase_Detail_LisrState extends State<Purchase_Detail_Lisr> {
  ...
  _fetchData() async {
    ...
    final response =
    await http.get('http://192.168.10.50:8081/api/Account/PurchaseSummeryReports?supplier=$cust_cod&Fromdate=$date_f&ToDate=$date_to&ItemCode=$item_cod&CurrentPage=1&PageSize=100');
    if (response.statusCode == 200) {
      Map jsonData = json.decode(response.body) ;
      Map jsonDat = jsonData['purchase'] ;
      List as = jsonDat['purchaseReport'];
                              ...
                              Row(
                                ...
                                children: <Widget>[
                                  Flexible(
                                    ...
                                    child: Row(
                                      children: <Widget>[
                                        ...
                                        SizedBox(width: 25,),
                                        Flexible(flex:1,child: SizedBox(child: Text((detail[index]['grN_DATE']),style: TextStyle(color: Colors.green),))),
                                      ],
                                    ),
                                  ),
                                  // SizedBox(width: 20,),
                                  Flexible(
                                    ...
                                    child: Row(
                                      children: <Widget>[
                                        Flexible(flex:1,child: SizedBox(child: Text('Supplier'),)),
                                        SizedBox(width: 20,),
                                        Expanded(flex:1,child: SizedBox(child: Text((detail[index]['suP_Name']), overflow: TextOverflow.clip,maxLines: 3,))),
                                      ],
                                    ),
                                  )
                                ],
                                  ...
                                  Flexible(
                                    ...
                                    child: Row(
                                      ...
                                      children: <Widget>[
                                        ...
                                        SizedBox(width: 20,),
                                        Flexible(flex:1,child: SizedBox(child: Text((detail[index]['grN_CODE'])))),
                                      ],
                                    ),
                                  ),
                                  //  SizedBox(width: 20,),
